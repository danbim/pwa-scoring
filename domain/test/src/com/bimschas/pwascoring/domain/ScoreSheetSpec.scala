...
class ScoreSheetSpec extends WordSpecLike with Matchers with PropertyChecks with DomainGenerators {
  ...
  val scoreSheetInputGen: Gen[ScoreSheetInput] = for {
    ...
    countingJumpScores <- Gen.const(
      Random
        .shuffle(JumpType.values.toList)
  ...
  "ScoreSheet" must {
    "calculate effective score (non-empty)" in {
      ...
      forAll(scoreSheetInputGen) {
        case ScoreSheetInput(heatRules, countingWaveScores, countingJumpScores, nonCountingWaveScores, nonCountingJumpScores) =>
